...
требуемые действия с помощью нескольких видов межпроцессного взаимодействия в системе Linux. Файлы работы должны
собираться с помощью Makefile. Программы
...
- Двустороннее знакомство с помощью сигнала. Данный тип используется в случае, когда на момент соединения клиента,
  ...
  этого) передаётся id клиента в сессии, с помощью которого клиент получает нужные идентификаторы дескрипторов для
  связи.
...
- “Независимые”. Клиенты являются независимыми процессами, при запуске им передается pid хоста. Здесь необходимо
  использовать содержательные типы знакомства. Типы:
    ...
    - Один ко многим. Каждый хост работает (в теории) с неограниченным количеством клиентов. Для работы с каждым
      клиентом
...
6. Именованные каналы (mkfifo). TYPE_CODE - fifo. Работа с именованным каналом должна производиться в каждом процессе
   уже после создания дочернего (так, будто это несвязанные процессы). После окончания работы родительский процесс
   должен
...
1. Тематическая пошаговая игра “Волк и семеро козлят”. Хост - волк, клиент(ы) - козлята, n- количество козлят. На каждом
   ...
   число, полученное мертвым козленком, отличается не более, чем на 20/n - он воскресает. До подключения козлят ничего
   не
...
2. Локальный сетевой чат. Хост и клиенты - участники чата. Хост может как сам отправлять личные и общие сообщения в чат,
   так и передавать сообщения от других участников в общий чат или лично. Общие сообщения отображаются в чате (консоли)
   у всех
   подключенных участников, личные сообщения отображаются в чате только у отправителя и адресата. Если клиент не
   отправляет
   сообщения больше минуты и не является хостом, то ему отправляется SIGKILL для завершения. Сообщения в чате появляются
   в
